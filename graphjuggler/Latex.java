...
class Latex {
	...
	out.write("c}$\\circ$&"); newLine(out);
	for (int j=0; j<t.length; j++) {	    
	    out.write("$\\phi_{"+j+"}$& ");
	}
	out.write("\\\\\\hline\\hline"); newLine(out);
	for (int i=0; i<t.length; i++) {	    
	    out.write("$\\phi_{"+i+"}$& ");
	    for (int j=0; j<t.length; j++) {
	...
	out.write("\\end{tabular}"); newLine(out);
	} catch (IOException ex) {
	    ex.printStackTrace();
	}
    }
	...
	newLine(out);
	out.write("\\usepackage{amsmath}\\usepackage{pstricks}\\usepackage{pst-node}\\pagestyle{empty}"); 
	newLine(out);
	...
	try {
	out.write("\\end{document}"); newLine(out);
	} catch (IOException ex) {
	    ex.printStackTrace();
	}
    }
	...
	out.write("\\end{bmatrix}"); newLine(out);
	} catch (IOException ex) {
	    ex.printStackTrace();
	}
    }
	...
	try {
	out.write("\\ \\\\$|$"+name+"$(G)|=$"+g.size()); newLine(out);
	} catch (IOException ex) {
	    ex.printStackTrace();
	}
    }
